    ...
    <div class="row">
        ...
        <div v-if="marks_sub_pats.length > 0"  class="col-md-2">
            <div v-for="(sub_pat, index) in marks_sub_pats" class="form-check">
                <span v-if="selected_pats.includes(sub_pat.id)">Founnd @{{ sub_pat.name }}</span>
                <span v-else>Not In @{{ sub_pat.name }}</span>
                {{--  <template v-if="selected_pats.includes(sub_pat.id)">
